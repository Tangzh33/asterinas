...
impl PacketState {
    const fn new() -> Self {
        Self {
            state: PacketCollectionState::WaitingForFirstByte,
    ...
    fn process_byte(&mut self, data: u8) -> Option<Vec<InputEvent>> {
        ...
        match self.state {
            PacketCollectionState::WaitingForFirstByte => {
                // First byte must have bit 3 set (always 1 in PS/2 mouse packets).
                if data & PACKET_SYNC_MASK != 0 {
                    self.packet[0] = data;
                    self.byte_index = 1;
                    self.state = PacketCollectionState::CollectingPacket;
                }
                None
            }
            PacketCollectionState::CollectingPacket => {
    ...
    fn reset(&mut self) {
        self.state = PacketCollectionState::WaitingForFirstByte;
        self.byte_index = 0;
        self.packet = [0; MouseType::PACKET_LEN_MAX];
    }
}
