...
pub fn sys_shmdt(addr: Vaddr, ctx: &Context) -> Result<SyscallReturn> {
    ...
    let shmid: u64 = root_vmar.get_shm_id(addr as usize)?;
    ...
    let len = (shm_obj.size() as usize).align_up(PAGE_SIZE);
    ...
    if shm_obj.should_be_deleted() {
        manager.try_delete_shm_obj(shmid as u64)?;
    }
