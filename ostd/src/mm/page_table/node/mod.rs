...
impl<E: PageTableEntryTrait, C: PagingConstsTrait> PageTableLock<E, C> {
    ...
    pub(super) fn alloc_marked(level: PagingLevel, token: Token) -> Self {
        let meta = PageTablePageMeta::new_locked(level, MapTrackingStatus::Tracked);
        let frame = FrameAllocOptions::new()
            .zeroed(false)
            .alloc_frame_with(meta)
            .expect("Failed to allocate a page table node");
        let frame_ptr = paddr_to_vaddr(frame.start_paddr()) as *mut E;
        let pte = E::new_token(token);
        // Fill it with tokens.
